...
def spline_filter_mat(n, order=3, border="mirror", trans=False, scipy=False):
	...
	for i in range(n):
		...
		data[i] = 1
		if not scipy:
			out[i] = interpol.spline_filter(data, order=order, border=border, trans=trans)
		else:
			if order > 1:
...
def interpol_mat(n, m, mode="spline", order=3, border="mirror", trans=False, scipy=False, deriv=False):
	...
	if not trans:
		...
		for i in range(m):
			...
			idata[i] = 1
			if not scipy:
				interpol.map_coordinates(idata, point, odata, mode=mode, order=order, border=border, trans=trans)
			else:
