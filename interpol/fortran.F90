...
! and dims(nx,ny,...) is the dimensionality of the unflattened data set,
! extract width(ndim) points in each dimension, producing an output
! out(:,nw,npoint) with nw = product(width). points(ndim,npoint) describes the
! position of each point in pixel units. The dimensions are assumed to
! be flattened in C order.
subroutine get_point_context(data, dims, points, width, cyclic, dir, out)
	implicit none
	real(_), intent(inout) :: data(:,:), out(:,:,:)
	real(_), intent(in)    :: points(:,:)
	integer, intent(in)    :: dims(:), width(:), dir
	logical, intent(in)    :: cyclic
	integer :: i, j, off(size(dims)), base(size(dims)), ndim, npoint, ind, oind, dind
	! In order to handle arbitrary dimensionality, we must
	! count up the nd loops ourselves.
	ndim   = size(points,1)
	npoint = size(points,2)
	do i = 1, npoint
		base = floor(points(:,i)-(width-2)*0.5d0)
		off  = 0
		! Loop through all cells in vincinity of point
		oind = 0
		do
			...
			do j = 1, ndim
				ind = base(j)+off(j)
				if(ind < 0 .or. ind >= dims(j)) then
					if(cyclic) then
						ind = modulo(ind, dims(j))
					else
						! Out of bounds - abort this point
						if(dir > 0) out(:,oind+1,i) = 0
						goto 1
					end if
				end if
				dind = dind*dims(j) + ind
			end do
			! Copy over the values
			if(dir > 0) then
				out(:,oind+1,i) = data(:,dind+1)
			else
				data(:,dind+1) = data(:,dind+1) + out(:,oind+1,i)
			end if
			1 oind = oind+1
			! Generate next indices
			do j = ndim, 1, -1
				off(j) = off(j)+1
				if(off(j)<width(j)) exit
				off(j) = 0
			end do
			if(j == 0 .and. off(1) == 0) exit
		end do
	end do
end subroutine

! Given points(ndim,npoint), a width=2 context(:,nw,npoint), perform
! multilinear interpolation and store it in out(:,npoint)
subroutine interpol_context_lin(points, context, dir, out)
	implicit none
	real(_), intent(in)    :: points(:,:)
	real(_), intent(inout) :: context(:,:,:), out(:,:)
	integer, intent(in)    :: dir
	real(_), allocatable   :: a(:,:)
	real(_)                :: v(size(context,1),2), x
	integer                :: ndim, npoint, i, j, k, na
	ndim   = size(points,1)
	npoint = size(points,2)
	allocate(a(size(context,1),size(context,2)))
	if(dir > 0) then
		do i = 1, npoint
			a  = context(:,:,i)
			na = size(a,2)
			! For each dimension, compute linear average.
			! a is in C ordering, so our 2 context points
			! in the last dimension form pairs. After merging
			! them, this is then true for the second to last
			! dimension, and so on.
			do j = ndim, 1, -1
				x = points(j,i)-floor(points(j,i))
				na= na/2
				do k = 0, na-1
					v(:,1) = a(:,2*k+1); v(:,2) = a(:,2*k+2)
					a(:,k+1) = (1-x)*v(:,1) + x*v(:,2)
				end do
			end do
			out(:,i) = a(:,1)
...
subroutine interpol_weights_poly(order, px, L)
	implicit none
	...
	real(_) :: dx
	n    = order+1
	base = floor(px-(n-2)*0.5d0)
	do i = 1, n
		dx = abs(base+i-1-px)
		L(i) = 0
		select case(order)
			case(0); L(i) = 1
			case(1); L(i) = max(1-dx,0d0)
			case(3)
				if(dx < 1) then
					L(i) = 1.5*dx**3 - 2.5*dx**2+1
				elseif(dx < 2) then
					L(i) = -0.5*dx**3+2.5*dx**2-4*dx+2
				end if
			case default;
				L(i) = 0
		end select
...
subroutine interpol_context_poly(points, context, order, dir, out)
	implicit none
	real(_), intent(in)    :: points(:,:)
	real(_), intent(inout) :: context(:,:,:), out(:,:)
	integer, intent(in)    :: dir, order(:)
	real(_), allocatable   :: a(:,:), L(:)
	integer                :: ndim, npoint, i, j, na, dim, n, set
	ndim   = size(points,1)
	npoint = size(points,2)
	allocate(a(size(context,1),size(context,2)))
	allocate(L(maxval(order)+1))
	if(dir > 0) then
		do i = 1, npoint
			a  = context(:,:,i)
			na = size(a,2)
			do dim = ndim, 1, -1
				n    = order(dim)+1
				call interpol_weights_poly(order(dim), points(dim,i), L)
				! loop through each set of context points
				do set = 1, na/n
					do j = 1, size(a,1)
						a(j,set) = sum(a(j,(set-1)*n+1:set*n)*L(1:n))
					end do
				end do
				na = na/n
			end do
		...
		do i = 1, npoint
			...
			do dim = 1, ndim
				...
				do set = na/n, 1, -1
					do j = 1, size(a,1)
						a(j,(set-1)*n+1:set*n) = a(j,set)*L(1:n)
					end do
				end do
				na = na*n
			end do
...
subroutine interpol_weights_lanczos(order, px, L)
	implicit none
	integer, intent(in) :: order
	real(_), intent(in) :: px
	real(_), intent(out):: L(:)
...
subroutine interpol_context_lanczos(points, context, order, dir, out)
	implicit none
	real(_), intent(in)    :: points(:,:)
	real(_), intent(inout) :: context(:,:,:), out(:,:)
	integer, intent(in)    :: dir, order(:)
	...
	npoint = size(points,2)
	allocate(a(size(context,1),size(context,2)))
	! Lanczos order indicates the number of right-hand zeros to include.
	! The zeros occur at input sample locations (excluding the center),
	! so we need 2*order+1 data points of context.
	allocate(L(2*maxval(order)+1))
	if(dir > 0) then
		do i = 1, npoint
			a  = context(:,:,i)
			na = size(a,2)
			do dim = ndim, 1, -1
				n = 2*order(dim)+1
				call interpol_weights_lanczos(order(dim), points(dim,i), L)
				! loop through each set of context points
				do set = 1, na/n
					do j = 1, size(a,1)
						a(j,set) = sum(a(j,(set-1)*n+1:set*n)*L(1:n))
...
!subroutine calc_weights2(type, order, p, weights, off)
!	implicit none
...
!	integer :: ndim, nw, i, j
!	real(_) :: x(size(weights,1),size(weights,2))
!	ndim = size(weights, 2)
!	nw   = size(weights, 1)
...
!	end if
!	! Offset of first sample of context relative to floor(p).
!	do i = 1, ndim
!		off(i) = floor(p(i)-(nw-2)*0.5d0)
!		do j = 1, nw
!			x(j,i) = abs(p(i)-(j-1)-off(i))
!		end do
!	end do
!	weights = 0
!	select case(type)
!		case(0) ! convolution
!			select case(order)
!				case(0); where(x < 0.5) weights = 1
!				case(1); where(x < 1.0) weights = 1-x
!				case(3)
!					where    (x < 1); weights =  1.5*x**3 - 2.5*x**2 + 1
!					elsewhere(x < 2); weights = -0.5*x**3 + 2.5*x**2 - 4*x + 2; end where
!			end select
!		case(1) ! spline
!			select case(order)
!				case(0); where(x < 0.5) weights = 1
!				case(1); where(x < 1.0) weights = 1-x
!				case(2)
!					where    (x < 0.5); weights = 0.75-x**2
!					elsewhere(x < 1.5); weights = 0.50*(1.5-x)**2; end where
!				case(3)
!					where    (x < 1.0); weights = (x*x*(x-2)*3+4)/6
!					elsewhere(x < 2.0); weights = (2-x)**3/6; end where
!				case(4)
!					where    (x < 0.5); weights = x**2 * (x**2 * 0.25-0.625)+115d0/192
!					elsewhere(x < 1.5); weights = x*(x*(x*(5/6-x/6)-1.25)+5d0/24)+55d0/96
!					elsewhere(x < 2.5); weights = (x-2.5)**4/24; end where
!				case(5)
!					where    (x < 1.0); weights = x**2*(x**2*(0.25-x/12)-0.5)+0.55
!					elsewhere(x < 2.0); weights = x*(x*(x*(x*(x/24-0.375)+1.25)-1.75)+0.625)+0.425
!					elsewhere(x < 3.0); weights = (3-x)**2 * x**2 / 120; end where
!			end select
!		case(2) ! lanczos
!			if(order == 0) then
!				where(x < 0.5) weights = 1
!			else
!				do i = 1, ndim
!					do j = 1, nw
!						if(x(j,i) < order) weights(j,i) = sinc(x(j,i))*sinc(x(j,i)/order)
!					end do
!				end do
!			end if
!	end select
...
subroutine calc_weights(type, order, p, weights, off)
	implicit none
	integer, intent(in)    :: type, order
	integer, intent(inout) :: off(:)
	real(_), intent(in)    :: p(:)
