...
contains
	...
	subroutine nmat_detvecs(ftod, bins, iNu, V, E, vbins)
		...
		real(4),    intent(in)    :: iNu(:,:), V(:,:), E(:)
		! Work
		complex(4), allocatable   :: Q(:,:), Qd(:,:), orig(:,:), iNud(:,:)
		complex(4)                :: esign
		...
		do bi = nbin, 1, -1
			...
			if(nv > 0) then
				...
				call cgemm('T', 'T', nv, nf, ndet, (1.0,0.0), Q, ndet, orig, nf,   (0.0,0.0), Qd, nv)
				call cgemm('T', 'T', nf, ndet, nv, esign,    Qd,   nv,    Q, ndet, (1.0,0.0), iNud, nf)
			end if
			do di = 1, ndet
	...
	subroutine nmat_covs(ftod, bins, covs)
		...
		real(4),    intent(in)    :: covs(:,:,:)
		! Work
		complex(4), allocatable   :: cC(:,:), orig(:,:)
		...
		do bi = nbin, 1, -1
			...
			call cgemm('N', 'N', nf, ndet, ndet, (1.0,0.0), orig, nf, cC, ndet, (0.0,0.0), ftod(b1:b2,:), nf)
			deallocate(cC, orig)
		end do
	end subroutine
