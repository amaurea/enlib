...
class SignalSrcSamp(SignalCut):
	def __init__(self, scans, dtype, comm, srcs=None, tol=100, amplim=None, rel=False, src_mask=None, name="srcsamp", ofmt="{name}_{rank:02}", output=False, cut_type="full"):
		Signal.__init__(self, name, ofmt, output, ext="hdf")
		self.data  = {}
		self.dtype = dtype
		cutrange = [0,0]
		for scan in scans:
			...
			mask	 = np.zeros([scan.ndet,scan.nsamp], bool)
			tod      = np.zeros((scan.ndet,scan.nsamp), np.float32)
			psrc.forward(tod, srcparam, tmul=0)
			mask    |= tod > tol
			if src_mask is not None:
				pmap  = pmat.PmatMap(scan, src_mask)
				pmap.forward(tod, src_mask, tmul=0)
				mask |= tod > 0.5
			del tod
