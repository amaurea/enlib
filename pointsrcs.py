...
def sim_srcs(shape, wcs, srcs, beam, omap=None, dtype=None, nsigma=5, rmax=None, method="loop", mmul=1,
	...
	if method == "vectorized":
		model = eval_srcs_vectorized(posmap, poss, amps, beam, cres, nhit, cell_srcs)
	else:
...
# This one is pretty pointless. It uses more memory than the looping one, and
...
def eval_srcs_vectorized(posmap, poss, amps, beam, cres, nhit, cell_srcs):
	...
	bpix   = (diff[0]**2 + (diff[1]*np.cos(cposmap[0,:,:,None,:,:]))**2)**0.5
