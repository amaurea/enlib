module pmat_core
	...
	private map_block_prepare_new, map_block_finish_new
	private map_block_prepare_shifted_flat_new, map_block_finish_shifted_flat_new
...
!    order mapmaking because it isn't shifted.
	...
	subroutine pmat_map_get_pix_poly_shift( &
		...
		implicit none
		! Parameters
		integer(4), intent(in)    :: wbox(:,:), wshift(:,:), nphi, sdir(:)
	...
	subroutine build_pointing_int_poly_shift(bore, hwp, pix, phase, det_comps, coeff, sdir, wbox, wshift)
		...
		integer(4) :: nsamp, xind(3), ig, ic, si, nwx, nwy, iwx, iwy
		logical    :: use_hwp
		real(8)    :: az, t
		nsamp = size(bore,2)
	...
	subroutine pmat_int_get_pix_flat( &
			pmet,                      &! pointing. 1: bilinear, 2: gradient
			mmet,                      &! binning.  1: nearest,  2: bilinear
			pix,  phase,               &! Main inputs/outpus
			bore, hwp, det_comps,      &! Input pointing
		...
		!$omp parallel do
		do di = 1, ndet
			call build_shifted_pix_poly_int_flat(pmet, bore, hwp, pix(:,di), phase(:,:,di), det_comps(:,di), coeffs(:,:,di), sdir, wbox, wshift)
		end do
	end subroutine

	subroutine pmat_int_use_pix_flat( &
			dir,                       &! Direction of the projection: 1: forward (map2tod), -1: backard (tod2map)
			mmet,                      &! binning.  1: nearest,  2: bilinear
			tmul, mmul,                &! Consts to multiply tod/map by
			tod, map,                  &! Main inputs/outpus
		...
		do di = 1, ndet
			...
			! 0.0648 / 0.1714, tod2map slower due to atomic, but separate buffers
			! is even slower for nthread > 8
			case(1); call project_map_nearest_int_flat (dir, tmul, tod(:,di), wmap, pix(:,di), phase(:,:,di))
			end select
			tloc1 = omp_get_wtime()
			tproj = tproj + tloc1-tloc2
		end do
	...
	subroutine project_map_nearest_int_flat( &
		...
		elseif(tmul .ne. 0) then
			if (omp_get_max_threads() > 1) then
				...
				do si = 1, nsamp
					p = pix(si)
					map(1,p) = map(1,p) + tod(si)*tmul
					do ci = 2, 3
						map(ci,p) = map(ci,p) + (tod(si)*tmul)*phase(ci-1,si)
					end do
				end do
			end if
		end if
	...
	subroutine build_shifted_pix_poly_int_flat(pmet, bore, hwp, pix, phase, det_comps, coeff, sdir, wbox, wshift)
		implicit none
		integer(4), intent(in)    :: pmet
		real(8),    intent(in)    :: bore(:,:), hwp(:,:), det_comps(:), coeff(:,:)
		integer(4), intent(inout) :: pix(:)
